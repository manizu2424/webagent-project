...
아래 완료 표시는 기본 구현과 2026-09-07까지의 검증 이력입니다. 전체 검토에서 발견한 R1~R5는 수정 완료됐으며, 남은 출시 전 보완 사항은 아래 `전체 프로젝트 검토 및 개선 작업`에서 관리합니다.
...
> **현재 위치: R1~R5 및 4A 로컬 왕복 완료, R6 저장 일관성 착수 전입니다.**
>
> R6 저장 일관성 → R7·R8 화면 복구·관리자 상담 순으로 진행합니다. 기존 1~3번의 기본 구현은 유지하되 새로 발견된 결함은 별도로 보완합니다.
...
- AI callback payload의 배열·객체 구조, 글자 수와 숫자 범위에 대한 기본 Zod schema를 구현했습니다. 숫자 `null` 허용과 단계 순서 중복·누락 검증은 R9에서 보완합니다.
...
5. R6: 저장 transaction, 제출 중복 방지 및 로그 저장 실패의 응답 정책을 정리합니다. **다음 착수 항목입니다.**
6. R7·R8: 네트워크 오류 복구와 관리자 상담 상세·연락처·페이지 이동을 구현합니다.
7. R9·R10을 보완하고 결과 화면(5번), Telegram(6번), 핵심 E2E(7번)를 진행합니다. 실제 AI 연결(4B)은 공개 운영 전에 완료합니다.
8. 운영 이미지(8번)를 검증하고, 운영 환경 준비(9번)와 백업·복원(10번)을 완료한 뒤 공개 운영합니다. 문서 동기화는 각 작업 완료 시 함께 진행합니다.
...
- [ ] fetch 또는 JSON 파싱 실패 시 오류 UI와 재시도 동작 추가
- [ ] 요청 timeout 및 polling 최대 대기 시간 적용, 장기 대기 시 수동 재조회·상담 안내
- [ ] `COMPLETED`이지만 결과가 없는 불일치 상태에서 점수 `0`을 표시하지 않도록 처리
- [ ] 예상 절감 시간 표시
- [ ] 추천 자동화 업무 표시
- [ ] 난이도와 추천 기술 스택 표시
- [ ] 구현 단계 표시
...
- [ ] 추천 업무와 구현 단계의 공개 DTO를 구체적인 타입으로 정의
...
검토 결론: MVP 기본 구조는 갖췄으나 공개 운영 전 기능 오류, 데이터 일관성, 보안 및 운영 준비 보완이 필요합니다. 아래 항목 중 R1~R5는 수정과 검증을 완료했고, 나머지는 아직 코드에 반영하지 않았습니다.
...
- 전체 검토 당시 35개 테스트에는 실제 폼의 빈 진단 ID, callback 선완료, 로그인 제한과 개인정보 없는 오류 로그 사례가 없었습니다. R2~R5 회귀 테스트를 추가해 현재 49개가 통과합니다.
...
- 관련 5개 파일·21개 테스트와 전체 12개 파일·49개 테스트 통과
...
- [ ] lead·diagnosis 및 독립 상담의 lead·consultation 저장을 각각 transaction으로 처리
- [ ] DB transaction 밖에서 외부 n8n 호출 수행
- [ ] 제출 중복 방지 키와 동일 요청 재전송 시 기존 결과 반환 정책 구현
- [ ] 핵심 저장 성공 후 실행 로그 저장 실패의 응답·복구 정책 정의
...
- [ ] 관련 폼·결과 조회의 예외 처리, 요청 timeout, 재시도와 상태 복구 구현
- [ ] 결과 polling의 최대 대기 시간과 장기 대기 안내 추가
- [ ] 결과 없는 완료 상태를 유효한 0점과 구분
- [ ] 관련 파일: `components/diagnosis/`, `components/consultation/consultation-form.tsx`, `components/admin/login-form.tsx`의 오류 사례 검증
...
- [ ] 상담 상세와 이메일·전화번호·요청 내용·희망 일정 표시
- [ ] 진단·상담 목록의 페이지 이동 구현
- [ ] 상담 메모 조회·편집 구현
- [ ] 진단 연결 시 기존 연락처 사용 또는 별도 연락처 저장 정책을 확정하고 폼과 API에 일관되게 반영
...
- [ ] AI 숫자 입력의 허용 타입을 명시하고 null·빈 문자열·boolean의 숫자 변환 차단
- [ ] 구현 단계의 1부터 시작하는 순서, 중복·누락 검사
- [ ] 잘못된 payload 거부와 정상 결과 저장 회귀 검증
...
- [ ] 운영 프록시에서 신뢰할 수 있는 클라이언트 IP 전달 규칙과 앱 직접 접근 제한 확정
- [ ] 만료 버킷 정리와 저장소 크기 제한 적용
- [ ] 다중 인스턴스 운영 시 제한 공유 방식 결정
- [ ] 위조 전달 헤더, 만료와 로그인·공개 API 제한 동작 검증
...
- [ ] 추천 업무·절감 시간·기술 스택·구현 단계 표시 (5번)
- [ ] 실제 AI 연결(4B), Telegram(6번), 운영 앱 Compose(8번), 백업·복원(10번) 완료
...
- [ ] 자동 백업과 실제 복원 검증
- [ ] 핵심 E2E 테스트 통과
- [x] lint, Vitest 49개, production build 통과 (2026-09-07 재검증)
